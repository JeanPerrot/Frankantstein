...
public class Ants {
    ...
    public Ilk getIlk(Tile tile) {
        return map.getIlk(tile.getRow(), tile.getCol());
    }
    ...
    public void clearMyAnts() {
        for (Tile myAnt : myAnts) {
            map.addKnowledge(myAnt, Ilk.LAND);
        }
        myAnts.clear();
    }
    ...
    public void clearFood() {
        for (Tile food : foodTiles) {
            map.addKnowledge(food, Ilk.LAND);
        }
        foodTiles.clear();
    }
    ...
    public void clearDeadAnts() {
        ...
        for (int row = 0; row < rows; row++) {
            for (int col = 0; col < cols; col++) {
                if (map.getIlk(row, col) == Ilk.DEAD) {
                    map.addKnowledge(new Tile(row, col), Ilk.LAND);
                }
            }
        }
    }
    ...
    public void markVisionExplored() {
        for (Tile tile : myAnts) {
            for (Tile offset : visionOffsets) {
                Tile visible = getTile(tile, offset);
                map.markExplored(visible.getRow(), visible.getCol());
            }
